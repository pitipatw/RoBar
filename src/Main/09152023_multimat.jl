...
server = WebSockets.listen!("127.0.0.1", 2000) do ws
    for msg in ws
        ...
        elseif stage == "Opt"
            ...
            amin = 0.0001 # minimum density
            V = data["maxVf"]

            a0 = fill(0.5, ncells) # initial area 
            # x0 = fill(0.5,ncells) # initial material
            ...
            solver = FEASolver(Direct, problem; xmin=amin)
            comp = TopOpt.Compliance(solver)

            function obj(x)
                # minimize compliance
                return sum(x .* elements_L) / sum(elements_L) 
                return 
            end
            println(obj(a0))
            function constr(x)
                # volume fraction constraint
                comp(PseudoDensities(x)) - V
            end
            ...
            addvar!(m, zeros(length(a0)), ones(length(a0)))
            # add constrain
            ...
            @time r = Nonconvex.optimize(
                m, MMA87(; dualoptimizer=ConjugateGradient()), a0; options=options
            )
