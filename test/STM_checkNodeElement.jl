...
"""
function checkNodeElement(node_element_index,node_element_area)
    #create a copy of the node_element_area to modify
    mod_node_element_index = deepcopy(node_element_index)
    mod_node_element_area = deepcopy(node_element_area)
    #loop each node
    for (k,v) in node_element_area
        #check if the number of the non-zera element connected to the node
        #is >= 3
        if sum(v.>0) < 3
            pop!(mod_node_element_area, k)
            pop!(mod_node_element_index, k)
    ...
    return mod_node_element_index, mod_node_element_area
end
